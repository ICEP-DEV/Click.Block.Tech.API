...
const SupportingDocumentService = require('../service/supportingDocumentService'); // Adjust the path as needed

// Create a new supporting document
router.post('/documents', async (req, res) => {
    const newDocument = req.body; // Assuming the new document data is sent in the request body
    try {
        // Call the service method to create a new document
        const insertId = await SupportingDocumentService.createSupportingDocument(newDocument);
        res.status(201).json({ message: 'Document created', id: insertId });
    } catch (err) {
